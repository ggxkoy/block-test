...
SEQUENCE_NAME = "LS_BlockRush_V5_StylePreview_R5_UIVolume_V2"
...
def make_textured_wood_material(
    ...
    world_x = expression(material, unreal.MaterialExpressionComponentMask, -830, -80)
    ...
    connect(world, "XYZ", world_x, "")
    world_y = expression(material, unreal.MaterialExpressionComponentMask, -830, 20)
    ...
    connect(world, "XYZ", world_y, "")

    wood_uv = expression(material, unreal.MaterialExpressionAppendVector, -650, -20)
    if swap_xy:
        connect(world_y, "", wood_uv, "A")
        connect(world_x, "", wood_uv, "B")
    else:
        connect(world_x, "", wood_uv, "A")
        connect(world_y, "", wood_uv, "B")

    scale = expression(material, unreal.MaterialExpressionConstant, -810, 110)
    scale.set_editor_property("r", uv_scale)
    scaled = expression(material, unreal.MaterialExpressionMultiply, -460, -20)
    connect(wood_uv, "", scaled, "A")
    connect(scale, "", scaled, "B")

    sample = expression(material, unreal.MaterialExpressionTextureSample, -250, -20)
    sample.set_editor_property("texture", WOOD_TEXTURE)
    connect(scaled, "", sample, "UVs")

    multiplier = expression(material, unreal.MaterialExpressionConstant3Vector, -250, 145)
    multiplier.set_editor_property(
        "constant", unreal.LinearColor(*texture_multiplier, 1.0)
    )
    tinted = expression(material, unreal.MaterialExpressionMultiply, -20, 0)
    connect(sample, "RGB", tinted, "A")
    connect(multiplier, "", tinted, "B")

    lift_node = expression(material, unreal.MaterialExpressionConstant3Vector, -20, 155)
    lift_node.set_editor_property("constant", unreal.LinearColor(*lift, 1.0))
    result = expression(material, unreal.MaterialExpressionAdd, 200, 0)
    connect(tinted, "", result, "A")
    connect(lift_node, "", result, "B")
    connect_property(result, "", unreal.MaterialProperty.MP_BASE_COLOR)

    rough = expression(material, unreal.MaterialExpressionConstant, 200, 130)
    rough.set_editor_property("r", roughness)
    connect_property(rough, "", unreal.MaterialProperty.MP_ROUGHNESS)
    spec = expression(material, unreal.MaterialExpressionConstant, 200, 220)
...
def build_fused_board(name, rows):
    ...
    finish_fused_mesh(final_mesh, 6.0)
    return create_static_mesh(final_mesh, name)


def build_fused_shape(name, cells, pitch=78.0, height=34.0, bevel=5.0):
...
SM_BUTTON = build_beveled_box("SM_V4_Button", (112.0, 112.0, 20.0), 5.5, 4)
SM_PLAQUE_FRAME = build_beveled_box("SM_V4_PlaqueFrame", (418.0, 150.0, 18.0), 5.0, 4)
SM_PLAQUE_FACE = build_beveled_box("SM_V4_PlaqueFace", (382.0, 116.0, 8.0), 2.8, 3)
SM_SEGMENT = build_beveled_box("SM_V4_DigitSegment", (40.0, 12.0, 8.0), 2.6, 3)
SM_ICON_BAR = build_beveled_box("SM_V4_IconBar", (44.0, 12.0, 8.0), 2.6, 3)
SM_GEAR_TOOTH = build_beveled_box("SM_V4_GearTooth", (24.0, 14.0, 8.0), 2.6, 3)
SM_COMBO_TAG_FRAME = build_beveled_box("SM_V4_ComboTagFrame", (242.0, 68.0, 10.0), 3.2, 3)
SM_COMBO_TAG_FACE = build_beveled_box("SM_V4_ComboTagFace", (224.0, 52.0, 6.0), 2.0, 3)
...
def spawn_text_pair(label, text, location, size, front_color, folder):
    ...
    for layer_index in range(layer_count):
        alpha = layer_index / float(layer_count)
        actors.append(
            spawn_text(
                "%s_Depth_%02d" % (label, layer_index),
                text,
                (
                    location[0] + 6.0 * (1.0 - alpha),
                    location[1] - 5.0 * (1.0 - alpha),
                    location[2] - 6.0 * (1.0 - alpha),
                ),
                size,
                (
                    int(55 + 55 * alpha),
...
# A conventional left arrow made from beveled, raised dark-wood bars.  The
# portrait camera mirrors world X, so this world-space arrow points screen-left.
for index, (offset, angle, length) in enumerate(
    ...
    spawn_mesh(
        ...
        SM_ICON_BAR,
        (455.0 + offset[0], 900.0 + offset[1], 109.0),
        (length / 44.0, 1.0, 1.0),
        M_INK,
...
# Settings gear: the teeth overlap a larger hub, avoiding the detached sun-ray
# silhouette of the previous pass.
spawn_mesh(
    "V4_SettingsHub",
    CYLINDER,
    (-455.0, 900.0, 113.0),
...
for index in range(8):
    ...
    spawn_mesh(
        ...
        SM_GEAR_TOOTH,
        (-455.0 + math.cos(radians) * 34.0, 900.0 + math.sin(radians) * 34.0, 109.0),
        (1.0, 1.0, 1.0),
        M_INK,
        rotation=(0.0, angle, 0.0),
        folder="BlockRushV4/UI/Buttons/Icons",
...
def spawn_plaque(prefix, center_x, face_material):
    ...
    face = spawn_mesh(
        prefix + "_InsetFace",
        SM_PLAQUE_FACE,
        (center_x, 762.0, BOARD_TOP_Z + 14.0),
        (1.0, 1.0, 1.0),
        face_material,
        folder="BlockRushV4/UI/Plaques",
    )
...
spawn_mesh(
    "V4_ComboTagFrame",
    SM_COMBO_TAG_FRAME,
    (-235.0, 870.0, BOARD_TOP_Z + 1.0),
    (1.0, 1.0, 1.0),
    M_BUTTON,
    folder="BlockRushV4/UI/ComboTag",
)
spawn_mesh(
    "V4_ComboTagFace",
    SM_COMBO_TAG_FACE,
    (-235.0, 870.0, BOARD_TOP_Z + 9.0),
    (1.0, 1.0, 1.0),
    M_DARK,
    folder="BlockRushV4/UI/ComboTag",
)
...
best_digits = spawn_number("V4_Best6864", "6864", 235.0, 762.0, 113.0, M_GOLD)
combo_digits = spawn_number("V4_Combo4120", "4120", -235.0, 762.0, 113.0, M_CREAM)

# Crown tier indicator uses a dark physical side under a raised gold face.
spawn_mesh(
    "V4_CrownIcon_Depth",
    SM_CROWN,
    (238.0, 905.0, BOARD_TOP_Z + 4.0),
    (1.0, 1.0, 1.0),
    M_INK,
...
key = unreal.EditorLevelLibrary.spawn_actor_from_class(
    ...
    unreal.Rotator(-70.0, 135.0, 0.0),
)
key.set_actor_label("V4_WarmKey")
key.set_folder_path("BlockRushV4/Lighting")
key.light_component.set_editor_property("intensity", 3.10)
...
set_if_supported(key.light_component, "light_source_angle", 14.0)
set_if_supported(key.light_component, "shadow_amount", 0.50)
...
# A restrained portrait tilt exposes the physical sides of buttons, digits,
# plaques and blocks without turning the board into an oblique perspective shot.
camera_location = unreal.Vector(0.0, -260.0, 3150.0)
...
if unreal.EditorAssetLibrary.does_asset_exist(SEQUENCE_PATH):
    ...
    if old_sequence:
        ...
        if asset_editor:
            asset_editor.close_all_editors_for_asset(old_sequence)
        del old_sequence
